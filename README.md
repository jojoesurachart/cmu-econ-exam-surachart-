# cmu-econ-exam-surachart-
เนื่องจากไฟล์ code มีขนาดใหญ่จึงไม่สามารถ upload ไฟล์ใส่ใน github ได้ผมจึงทำเป็นไฟล์ zip
#ลิงค์สำหรับไฟล์ code
  - https://drive.google.com/drive/folders/1AViqRhTliY3K5zFbadQZOjdea-s_i5bY?usp=sharing
#ภาษาที่ใช้
1. Laravel เป็น backend
2. reactJS เป็น frontend
3. ใช้ MySql เป็นฐานข้อมูล
ขั้นตอนการติดตั้งไฟล์
1. แตกไฟล์ออกมา
2. ติดตั้งฐานข้อมูลซึ่งไฟล์จะอยู่ที่ folder ชื่อว่า database > db_econ_cmu.sql
3. ดำเนินการเปิด cmd พิมพ์ command (สำหรับจำลอง  laravel) 
   econ_cmu/php artisan serv
4. ดำเนินการเปิด cmd พิมพ์ command (สำหรับจำลอง reactjs)
    econ_cmu/econ_cmu_system/npm run dev
5. Source reactjs จะอยู่ที่ Folder ที่ชื่อว่า 
    
   
